...
export function UploadListManager({ days, uploads, isLocked }: UploadListManagerProps) {
  const router = useRouter();
  const [forms, setForms] = useState<Record<string, { dayId: string; scheduleId: string; memo: string }>>(
    () =>
      Object.fromEntries(
        uploads.map((upload) => [
          upload.id,
  ...
  function updateForm(uploadId: string, patch: Partial<{ dayId: string; scheduleId: string; memo: string }>) {
    setForms((current) => ({
      ...current,
  ...
  if (uploads.length === 0) {
    return (
      <Card>
        <p className="text-secondary text-on-surface-variant">아직 내 업로드가 없습니다.</p>
      </Card>
    );
  }
      ...
      {uploads.map((upload) => {
        ...
        return (
          <Card key={upload.id} className="overflow-hidden p-0">
            <div className="grid gap-md lg:grid-cols-[280px_1fr]">
              <MediaPreview files={upload.files} className="h-full min-h-[220px] rounded-none" />
              <div className="space-y-md p-md">
                ...
                <div className="grid gap-sm lg:grid-cols-2">
                  ...
                  <select
                    ...
                    onChange={(event) => updateForm(upload.id, { scheduleId: event.target.value })}
                  >
