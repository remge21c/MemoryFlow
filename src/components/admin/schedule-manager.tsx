...
function displayTimeValue(value: string) {
  const matched = value.match(/^(\d{1,2}):00$/);
...
function QuickTimePicker({
  ...
  const hasOption = quickTimeOptions.some((option) => option.value === value);
  ...
  return (
    ...
    >
      ...
      {value && !hasOption ? <option value={value}>{displayTimeValue(value)}</option> : null}
      {quickTimeOptions.map((option) => (
        <option key={option.value || "empty-time"} value={option.value}>
          {option.label}
        </option>
      ))}
    </select>
